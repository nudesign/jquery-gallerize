  ...
  $.fn.gallerize = function(options) {
    ...
    return this.each(function() {
      ...
      var init = function () {
        ...
        if (settings.transition_fx === 'fade') {
      ...
      var defaultPaginator = function () {
        $paginator = $("<ul class='paginator' />");
        $children.each(function (i, el) {
          var $el = $(el);
          $el.data('index', i);
          if ($el.data('thumb_src') != undefined) {
            $paginator.append("<li data-index='" + i + "'><img src='" + $(el).data('thumb_src') + "'></li>");
          }
        });

        $paginator.bind('click.gallerize', function (e) {
        ...
        $children.parents('.gallery_window').append($paginator);
